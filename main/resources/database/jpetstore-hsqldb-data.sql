...
-- 持久化购物车表（用于基于数据库的购物车功能）
create table CART (
    username varchar(80) not null,
    itemid   varchar(10) not null,
    quantity int not null,
    constraint pk_cart primary key (username, itemid),
    constraint fk_cart_account foreign key (username) references ACCOUNT (userid),
    constraint fk_cart_item foreign key (itemid) references ITEM (itemid)
);

-- 用户行为日志表（用于记录登录、浏览、加购、下单等操作）
